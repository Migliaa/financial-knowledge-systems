# Ricerca e assistenti su documenti finanziari

Il [piano corrente](PIANO_MASTER.md) organizza apprendimento e portfolio in tre macroprogetti: **ricerca ed embedding → RAG personale e caching → confronto con ricerca agentica e LLM Wiki**.

Andrea è l'utilizzatore. Ogni parte ha una consegna autonoma, riusa il lavoro precedente e si chiude con risultati verificabili. Non occorrono clienti o feedback esterni. Prima comprendiamo il problema e il codice; poi allarghiamo il sistema.

| Necessità | Documento |
|---|---|
| Riprendere con Sol | [HANDOVER](HANDOVER.md), seguendo [AGENTS](AGENTS.md) |
| Focus M1 e repository da collegare | [Repository e responsabilità](docs/REPOSITORY.md) |
| Programma, esperimenti e criteri di chiusura | [PIANO_MASTER](PIANO_MASTER.md) |
| Competenze, strumenti ed esercizi di verifica | [Matrice competenze](percorso/COMPETENZE.md) |
| Mockup del portfolio e organizzazione pubblica | [Proposta](docs/PORTFOLIO.md), [repository mockup separata](https://github.com/Migliaa/portfolio-redesign) |
| Partire con Sol senza comandi speciali | [Avvio del corso](INIZIA_CON_SOL.md) |
| Primo incontro da zero | [Il problema](percorso/00-il-problema.md) |
| Metodo didattico e corsi mirati | [Percorso](percorso/README.md), [KodeKloud](percorso/KODEKLOUD.md) |
| Vincoli tecnici e controllo CPU | [Architettura](docs/ARCHITETTURA.md) |
| Prove, report e figure | [Documentazione](docs/DOCUMENTAZIONE.md), [scheda esperimento](esperimenti/TEMPLATE.md) |
| Consegna della direzione alla centralina | [Nota pronta, non inviata](NOTA_CENTRALINA.md) |
| Storia delle decisioni | [Diario](DIARIO.md), [analisi strategica precedente](STRATEGIA_PROFILO.md) |

**Stato: pianificazione pronta, U00 ancora da iniziare.** Nessuna pipeline, dato scaricato, misura o submission. Le cartelle di codice e risultati verranno create quando servono.

Partiamo da MTRAG FiQA, testi finanziari con annotazioni esistenti. Non è un flusso di prezzi o un sistema di trading: le risposte sono riferite a fonti congelate. Budget iniziale zero euro aggiuntivi; 24 GB RAM, Intel Iris Xe; fattibilità CPU da misurare. Alternativa di ricerca più leggera già autorizzata.

BRIEF, SCELTA, IMPOSTAZIONE e STRATEGIA_PROFILO conservano le decisioni precedenti. In caso di differenze nella sequenza, prevale PIANO_MASTER e, sempre, la richiesta più recente di Andrea.
